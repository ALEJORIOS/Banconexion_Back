...
app.get("/filtered-transactions", async(req: Request, res: Response) => {
    await dBConnection.sql`SELECT t.id, t.name, t.document_type, t.document, t.value, t.date, t.authorized_by, t.confirmed  FROM transactionsView t LEFT JOIN persons p ON t."userID" = p.id WHERE ("userID" = ${req.query.id as string} OR ${req.query.id as string} = ANY (PARENT_RELATIONSHIP));`
    .then((response) => {
        res.statusCode = 200;
        res.send(response.map(res => upperize(res)))
    })
